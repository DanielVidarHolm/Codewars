...
function isValidChess960(pieces) {
  ...
  let bishopAlternating = false;
  let kingBeetweenRooks = false;

  const bishopIndexArr = [];
  const rookKingKeyArr = [];

  //Gathering the pieces into readable data
  piecesArr.forEach((piece, i) => {
    if (piece === "B") {
      bishopIndexArr.push(i);
    }
    if (piece === "R" || piece === "K") {
      rookKingKeyArr.push(piece);
    }
  });

  // Checking if bishops are alternating with modulo of 2
  if (bishopIndexArr[0] % 2 === 0 && bishopIndexArr[1] % 2 === 1) {
    bishopAlternating = true;
  } else if (bishopIndexArr[1] % 2 === 0 && bishopIndexArr[0] % 2 === 1) {
    bishopAlternating = true;
  } else {
    bishopAlternating = false;
  }

  // checking if king is between rooks
  // since i pushed the keys in order i can just check if king is between the rooks in the rookKingKeyArr
  if (
    rookKingKeyArr[0] === "K" ||
    rookKingKeyArr[rookKingKeyArr.length - 1] === "K"
  ) {
    kingBeetweenRooks = false;
  } else {
    kingBeetweenRooks = true;
  }
  return bishopAlternating && kingBeetweenRooks;
}
