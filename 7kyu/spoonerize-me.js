...
function spoonerize(words) {
    ...
    let firstWordletterSplit = wordsSplit[0].split('')
    let secondWordletterSplit = wordsSplit[1].split('')
    
    let firstLetterFirstWord = firstWordletterSplit.shift()
    let firstLetterSecondWord = secondWordletterSplit.shift()
    
    firstWordletterSplit.unshift(firstLetterSecondWord)
    secondWordletterSplit.unshift(firstLetterFirstWord)
    
    let firstWord = firstWordletterSplit.join('')
    let secondWord = secondWordletterSplit.join('')
  
    let spoonerized = `${firstWord} ${secondWord}`
  ...
  function spoonerize(words) {
    ...
    let sub1 = wordSplit[0].substr(0,1)
    let sub2 = wordSplit[1].substr(0,1)
    return `${wordSplit[0].replace(sub1, sub2)} ${wordSplit[1].replace(sub2, sub1)}`
  }